...
if(isset($_POST['update'])){
	...
	if(!$data['name'] || !$data['price'] || !$data['description']){
		...
		if(($data['name'] == $product_info[0]['name']) && ($data['price'] == $product_info[0]['price']) && ($data['description'] == $product_info[0]['description'])){
			...
			if(updateProduct($db, $product_id, $data['name'], $data['price'], $data['description'])){
			...
			if(($prod_response = sendData($db ,'products', $products_json, 'http://127.0.0.1/billing_v1/get_test/test_get_products.php')) && !preg_match('/not found/', $prod_response)){
...
<form id="update_form" method="POST">
	...
	<input type="text" name="description" value="<?= $product_info['0']['description']; ?>" /><br />
	<?php else : ?>
