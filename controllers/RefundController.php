...
class RefundController extends Zend_Controller_Action{
	...
	public function indexAction(){
		...
		$this->view->ref_response = array('data' => null);
		
			/*
			$refund = $Data->receiveData($_POST['refund'], $this->db, $Key, true);
			if($refund === false){
				$response['data'] = 'Sorry, but you are not the one I have expected for.';
				return;
			}
			else{
				$response['data'] = 'Okay';
			...
			*/
			...
			$refund_json = '{"keys":[{"keyId":"115","orderId":"50","status":"0","percent":"172.99","id":"3"}],"percent":"8.00","refundID":"70"}';
			...
			if(is_string($refund_order)){
				...
				$this->view->ref_response = "{'status':'notexists'; 'id_keys': {$refund_order}; 'id_refund': {$refund['refund_id']}; 'success': false}";
				$Data_trial->sendData($this->db, 'refunds', "{'status':'notexists'; 'id_keys': {$refund_order}; 'id_refund': {$refund['refund_id']}; 'success': false}", '10.55.33.27/dev/receiveRefundResponse.php');
				return;
			}
			elseif(is_array($refund_order)){
				...
				if(is_string($inserted)){
					...
					$this->view->ref_response = "{'status': 'canceled', 'id_keys': {$inserted}, 'id_refund': {$refund['refund_id']}, 'success': false}";
					$Data_trial->sendData($this->db, 'refunds', "{'status': 'canceled', 'id_keys': {$inserted}, 'id_refund': {$refund['refund_id']}, 'success': false}", '10.55.33.27/dev/receiveRefundResponse.php');
					return;
				}
				elseif(is_bool($inserted)){
					...
					if(is_null($ref_res['refund'])){
						...
						$this->view->ref_response = "{'status': 'all', 'success': false,  'id_keys': [], 'id_refund': {$refund['refund_id']}}, 'success': false}";
						$Data_trial->sendData($this->db, 'refunds', "{'status': 'all', 'success': false,  'id_keys': [], 'id_refund': {$refund['refund_id']}}, 'success': false}", '10.55.33.27/dev/receiveRefundResponse.php');
					}
					else{
						...
						if(($PP_response = $Data_trial->sendData($this->db, 'refunds', $refunds_data_j, 'http://10.55.33.21/without_routing/discard.php')) && !preg_match('/not found/', $PP_response)){
							$this->view->ref_response = $PP_response;
						...
						$Data_trial->sendData($this->db, 'refunds', "{'success': true, 'id_keys': {$id_keys}, 'status': 'OK', 'id_refund': {$refund['refund_id']}, 'payment': {$PP_response}}", '10.55.33.27/dev/receiveRefundResponse.php');	
					}
				}
			}
