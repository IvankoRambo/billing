...
try{
    ...
    class SidebarLayoutPlugin extends Zend_Controller_Plugin_Abstract{
        ...
        public function preDispatch(Zend_Controller_Request_Abstract $request){
			...
			$view->router = $frontController->getRouter();            $frontController = Zend_Controller_Front::getInstance();
	...
	$routes->addRoute('getOrder', 
		new Zend_Controller_Router_Route('getOrder', array(
			'controller' => 'getorder',
			'action' => 'index')));
	$routes->addRoute('sendOrder', 
		new Zend_Controller_Router_Route('sendOrder', array(
